...
impl Provider for Docker {
    ...
    fn identify(&self, tx: SyncSender<ProviderId>, _timeout: Duration) {
        ...
        if self.check_dockerenv_file(DOCKERENV_FILE) || self.check_cgroup_file(CGROUP_FILE) {
...
impl Docker {
    ...
    #[instrument(skip_all)]
    fn check_dockerenv_file<P: AsRef<Path>>(&self, dockerenv_file: P) -> bool {
        debug!(
    ...
    #[instrument(skip_all)]
    fn check_cgroup_file<P: AsRef<Path>>(&self, cgroup_file: P) -> bool {
        debug!(
        ...
        if cgroup_file.as_ref().is_file() {
            return match fs::read_to_string(cgroup_file) {
...
mod tests {
    ...
    #[test]
    fn test_check_dockerenv_file_success() -> Result<()> {
        let dockerenv_file = NamedTempFile::new()?;

        let provider = Docker;
        let result = provider.check_dockerenv_file(dockerenv_file.path());
    ...
    #[test]
    fn test_check_dockerenv_file_failure() {
        let provider = Docker;
        let result = provider.check_dockerenv_file("/nonexistent/.dockerenv");

        assert!(!result);
    }

    #[test]
    fn test_check_cgroup_file_success() -> Result<()> {
        let mut cgroup_file = NamedTempFile::new()?;
        cgroup_file.write_all(b"12:cpuset:/docker/0123456789abcdef")?;

        let provider = Docker;
        let result = provider.check_cgroup_file(cgroup_file.path());
    ...
    #[test]
    fn test_check_cgroup_file_failure() -> Result<()> {
        let mut cgroup_file = NamedTempFile::new()?;
