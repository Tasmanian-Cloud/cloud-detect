...
impl Provider for BinaryLane {
    ...
    async fn identify(&self, tx: Sender<ProviderId>, _timeout: Duration) {
        ...
        if self
            .check_vendor_files(SYS_VENDOR_FILE, PRODUCT_NAME_FILE, CHASSIS_ASSET_TAG_FILE)
            .await
        {
    ...
    async fn check_vendor_files<P: AsRef<Path>>(
        ...
        for vendor_file in [sys_vendor_file, product_name_file, chassis_asset_tag_file] {
            ...
            if vendor_file.as_ref().is_file() {
                match fs::read_to_string(vendor_file).await {
                    Ok(content) => {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_check_vendor_files_success() -> Result<()> {
