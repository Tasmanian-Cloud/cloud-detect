...
impl Provider for ProxmoxLxc {
    ...
    async fn identify(&self, tx: Sender<ProviderId>, _timeout: Duration) {
        ...
        if self
            .check_pve_managed_files([HOSTS_FILE, NETWORK_INTERFACES_FILE, RESOLV_CONF_FILE])
            .await
            || self
                .check_container_env(PID1_ENVIRON_FILE, SYSTEMD_CONTAINER_FILE)
                .await
        {
...
impl ProxmoxLxc {
    ...
    #[instrument(skip_all)]
    async fn check_pve_managed_files<P: AsRef<Path>>(&self, managed_files: [P; 3]) -> bool {
        for managed_file in managed_files {
            ...
            if managed_file.as_ref().is_file() {
                match fs::read_to_string(managed_file).await {
                    Ok(content) => {
    ...
    #[instrument(skip_all)]
    async fn check_container_env<P: AsRef<Path>>(
        &self,
        environ_file: P,
        ...
        if environ_file.as_ref().is_file() {
            match fs::read(environ_file).await {
                Ok(content) => {
                    if content
                        .split(|&byte| byte == 0)
        ...
        if systemd_container_file.as_ref().is_file() {
            match fs::read_to_string(systemd_container_file).await {
                Ok(content) => {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_check_pve_managed_files_success() -> Result<()> {
    ...
    #[tokio::test]
    async fn test_check_pve_managed_files_failure() -> Result<()> {
        let mut hosts_file = NamedTempFile::new()?;
        let interfaces_file = NamedTempFile::new()?;
        let resolv_conf_file = NamedTempFile::new()?;
    ...
    #[tokio::test]
    async fn test_check_container_env_environ_success() -> Result<()> {
        ...
        let provider = ProxmoxLxc;
        let result = provider
            .check_container_env(environ_file.path(), systemd_container_file.path())
            .await;
    ...
    #[tokio::test]
    async fn test_check_container_env_systemd_success() -> Result<()> {
    ...
    #[tokio::test]
    async fn test_check_container_env_failure() -> Result<()> {
        let mut environ_file = NamedTempFile::new()?;
        let mut systemd_container_file = NamedTempFile::new()?;
